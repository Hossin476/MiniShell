...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/22 19:17:25 by ykhourba          #+#    #+#             */
/*   Updated: 2023/06/22 19:17:26 by ykhourba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_valid_word(t_lsttoken *item)
{
	if (item && (item->token & (tk_exp | tk_sl_qt | tk_db_qt | tk_word))
		&& (item->next) && (item->next->token == tk_wt_s) && (item->next->next)
...
{
	if ((item->token != tk_wt_s) && item->next && item->next->token == tk_wt_s
		&& item->next->next && (item->next->next->token != tk_wt_s))
		return (1);
	return (0);
}
